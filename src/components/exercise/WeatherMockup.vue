...
onMounted(() => {
  renderCount.value = 1
  console.log(`[WeatherMockup] render count: ${renderCount.value}`)
})
...
<template>
  <div class="dashboard-wrapper">
    ...
    <section class="list-box">
      <h3>🏙️ 지역별 날씨 현황</h3>
      <p style="margin-bottom: 12px; color: #6c757d">리렌더 횟수: {{ renderCount }}</p>

      <div
        v-for="item in weatherList"
        :key="item.id"
        v-memo="[item.id, item.temp, item.status]"
        class="weather-card"
